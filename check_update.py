...
def gitconfig():
    ...
    try:
        ...
        config_write.set_value("user", "name", "Oscarr")
    
    try:
        email = config_read.get_value("user", "email")
    except:
        config_write.set_value("user", "email", "oscarr@fake.email")
...
def check_and_apply_update():
    ...
    if result.startswith('Already'):
        logging.info('No new version of Oscarr available.')
    elif result.startswith('Updating') or result.startswith('Merge made'):
        logging.info('Oscarr updated to latest version and need to be restarted.')
        updated()
    else:
        logging.info(result)
